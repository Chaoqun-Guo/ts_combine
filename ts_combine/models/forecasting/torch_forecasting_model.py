...
This file contains several abstract classes:
    ...
    * TorchForecastingModel is the super-class of all torch (deep learning) ts_combine forecasting models.
...
from ts_combine.logging import (
    get_logger,
...
from ts_combine.models.forecasting.forecasting_model import (
    ForecastingModel,
    GlobalForecastingModel,
)
from ts_combine.models.forecasting.pl_forecasting_module import PLForecastingModule
from ts_combine.timeseries import TimeSeries
from ts_combine.utils.data.inference_dataset import (
...
from ts_combine.utils.data.sequential_dataset import (
...
from ts_combine.utils.data.training_dataset import (
...
from ts_combine.utils.likelihood_models import Likelihood
from ts_combine.utils.torch import random_method
...
class TorchForecastingModel(GlobalForecastingModel, ABC):
    ...
    def __init__(
        ...
        add_encoders
            ...
            model creation.
            Read :meth:`SequentialEncoder <ts_combine.utils.data.encoders.SequentialEncoder>` to find out more about
    ...
    def _verify_static_covariates(self, static_covariates: Optional[pd.DataFrame]):
        ...
        if static_covariates is not None and self._supports_static_covariates():
            ...
            raise_if(
                ...
                "`ts_combine.dataprocessing.transformers.static_covariates_transformer.StaticCovariatesTransformer`.",
                logger,
            )
    ...
    def fit(
        ...
        calling :func:`fit_from_dataset()` with a custom :class:`ts_combine.utils.data.TrainingDataset`.
    ...
    def fit_from_dataset(
        ...
        """
        Train the model with a specific :class:`ts_combine.utils.data.TrainingDataset` instance.
    ...
    def predict_from_dataset(
        ...
        """
        This method allows for predicting with a specific :class:`ts_combine.utils.data.InferenceDataset` instance.
        These datasets implement a PyTorch ``Dataset``, and specify how the target and covariates are sliced
    ...
    def save(self, path: Optional[str] = None) -> None:
        ...
        Example for saving and loading a :class:`RNNModel`:
            ...
            .. code-block:: python
                ...
                from ts_combine.models import RNNModel
    ...
    def save_model(self, path: str) -> None:
        ...
        .. deprecated:: 0.21.0
            `TorchForecastingModel.save_model()` is deprecated and will be removed in a future ts_combine version.
            Please use `TorchForecastingModel.save()` instead.
        """

        save_warning = (
            "`TorchForecastingModel.save_model()` is deprecated and will be removed "
            "in a future ts_combine version. Please use `TorchForecastingModel.save()` instead."
        )
    ...
    def load_model(path: str) -> "TorchForecastingModel":
        ...
        .. deprecated:: 0.21.0
            `TorchForecastingModel.load_model()` is deprecated and will be removed in a future ts_combine version.
            Please use `TorchForecastingModel.load()` instead.
        """

        load_warning = (
            "`TorchForecastingModel.load_model()` is deprecated and will be removed "
            "in a future ts_combine version. Please use `TorchForecastingModel.load()` instead."
        )
